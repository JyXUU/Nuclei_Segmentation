...
def process_image(image_path, xml_mask_path, output_dir, step_output_dir):
    base_filename = os.path.splitext(os.path.basename(image_path))[0]
    image = cv2.imread(image_path)
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    # save_image(image_rgb, step_output_dir, f"{base_filename}_01_original.jpg")
    
    final_mask = parse_xml_boundary_mask(xml_mask_path, image.shape)
...
def process_folder(data_folder, xml_folder, output_folder, step_output_folder):
    nuclei_counts = {}
    for filename in os.listdir(data_folder):
        if filename.endswith('.tif'):
            image_path = os.path.join(data_folder, filename)
            xml_mask_filename = f"{os.path.splitext(filename)[0]}.xml"
            xml_mask_path = os.path.join(xml_folder, xml_mask_filename)
            base_filename = os.path.splitext(filename)[0]
            if os.path.exists(xml_mask_path):
                nuclei_counts[base_filename] = process_image(image_path, xml_mask_path, output_folder, step_output_folder)
            else:
                print(f"XML boundary mask not found for {filename}")
...
xml_folder = 'data/mannual_masks'  
output_folder = 'results/segmentation_results/mannual_mask'
step_output_folder = 'results/step_results/mannual_mask'

process_folder(data_folder, xml_folder, output_folder, step_output_folder)
